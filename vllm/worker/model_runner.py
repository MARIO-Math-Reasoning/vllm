...
class ModelRunner:
    ...
    def execute_model(
        ...
        )  # [token_len, hidden_state_size]

        # Compute the logits.
        logits = self.model.compute_logits(hidden_states, sampling_metadata)  # [batch, V]
        ...
        if hasattr(self.model.config, "value_model") and input_metadata.is_prompt and self.model.config.value_model:
            values = self.model.v_head(hidden_states)

            for idx, sequencegroupoutput in enumerate(output.outputs):
                sequencegroupoutput.value_estimate = values[input_metadata.seq_start_loc[idx+1] - 1].item()
